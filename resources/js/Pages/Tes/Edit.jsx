...
const Edit = ({ assignment, courses }) => {
  ...
  const { course_id, course, access_time, minimum_score, type, questions } = assignment;
  ...
  const { data, setData, patch, errors, processing } = useForm({
    ...
    questions: type == 'knowledge' ? questions.map(item => ({
      ...
      answers: item.answers.map(answer => ({
        id: answer.id,
        text: answer.text,
        is_correct: Boolean(Number(answer.is_correct))
      })),
    })) : [],
	});
  ...
  const submit = (e) => {
    e.preventDefault();
		patch(route('tests.update', assignment));
	}
